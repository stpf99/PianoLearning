...
class MidiPlayer(QThread):
    ...
    def __init__(self, note_events, rtmidi_output, get_tempo_func):
        super().__init__()
        self.note_events = note_events
        self.rtmidi_output = rtmidi_output
        self.get_tempo_func = get_tempo_func  # Funkcja zwracająca aktualne tempo
        self.is_playing = False
        self.base_tempo = 120  # Domyślne tempo bazowe (BPM)
    ...
    def run(self):
        ...
        for event in self.note_events:
            if not self.is_playing:
                break
            current_tempo = self.get_tempo_func()  # Pobierz aktualne tempo
            tempo_scale = self.base_tempo / current_tempo  # Skaluj czas względem bazowego tempa
            adjusted_time = event['time'] * tempo_scale
            elapsed = adjusted_time - (time.time() - start_time)
            if elapsed > 0:
                time.sleep(elapsed)
...
class PianoLearningApp(QMainWindow):
    def __init__(self):
        ...
        self.num_keys = 48
        self.start_note = 36
        self.stream_speed = 100  # Domyślne tempo w BPM
        ...
        self.base_tempo = 120  # Domyślne tempo bazowe dla skalowania
    ...
    def create_settings_panel(self):
        ...
        speed_layout.addWidget(QLabel("Tempo (BPM):"))
    ...
    def load_midi_file(self, file_path):
        try:
            ...
            self.base_tempo = tempo  # Ustaw tempo bazowe z pliku MIDI
    ...
    def start_playing(self):
        ...
        if self.is_previewing:
            self.midi_player = MidiPlayer(self.note_events, self.rtmidi_output, self.get_current_tempo)
    ...
    def generate_composition(self):
        ...
        try:
            ...
            self.midi_file = None
            self.base_tempo = 120  # Domyślne tempo dla generowanych utworów
            self.speed_slider.setValue(int(self.base_tempo))
            self.speed_value.setText(str(int(self.base_tempo)))
            self.stream_speed = int(self.base_tempo)
            self.note_canvas.set_stream_speed(self.stream_speed)
            self.play_button.setEnabled(True)
            self.preview_button.setEnabled(True)
...
class NoteStreamCanvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        tempo_scale = self.parent.base_tempo / self.stream_speed  # Skaluj wizualizację względem bazowego tempa
        time_start = self.current_position - (height / (self.stream_speed * tempo_scale))
        time_end = self.current_position + (height / (self.stream_speed * tempo_scale))
        note_order = {}
            ...
            if event['type'] == 'note_on' and event['time'] >= time_start:
                ...
                if note_off and note_off['time'] >= time_start:
                    ...
                    if 0 <= note < self.num_keys:
                        x = note * key_width
                        start_y = height - (event['time'] - self.current_position) * self.stream_speed * tempo_scale
                        end_y = height - (note_off['time'] - self.current_position) * self.stream_speed * tempo_scale
